...
export async function POST(req: NextRequest) {
  ...
  if (!Array.isArray(order) || order.length === 0 || !order.every((id) => Number.isInteger(id))) {
    return NextResponse.json({ error: "order must be a non-empty array of doctor ids" }, { status: 400 });
  }
  ...
  try {
    ...
    for (let i = 0; i < order.length; i++) {
      await client.query("UPDATE doctors SET sort_order = $1, updated_at = now() WHERE id = $2", [i, order[i]]);
    }
